...
//public class thiscodeisbroken {
@TeleOp(name = "encodersSuck")

public class EncodersSuck extends LinearOpMode {
  ...
  public void runOpMode() {//initializes motors and servos
    ...
    if (opModeIsActive()) {
      ...
      while (opModeIsActive()) {
        //Controls movement on gamepad 1
        if (gamepad1.right_stick_x < -0.1)
            turnLeft(0.1,0.7);
        if (gamepad1.right_stick_x > 0.1) { // right
          turnRight(0.1,0.7);
        }
        if (gamepad1.left_stick_y < -0.1) {//backward
          moveBackward(0.1, 0.7);
        }
        if (gamepad1.left_stick_y > 0.1) {//forward
          moveForward(0.1, 0.7);
        }
        if (gamepad1.right_bumper) {//right strafe
          strafeRight(0.1,0.7);
        }
        if (gamepad1.left_bumper) {//left strafe
          strafeLeft(0.1,0.7);
        }

        //Slide Arm Management - gamepad2
        ...
        /*frontleft.setVelocity(motorVelocity);//framework for method rewrite
        ...
        telemetry.addData("turner", "%.2f", turnerPosition);
        //telemetry.update();
      }
    }
  }
